...
# beagle file name
bglname="boreogadus_OZ177908.1_s26000000_e27500000"
...
cov <- as.matrix(read.table(paste0("./results/pca/",bglname,".cov")))
  ...
  order_cluster <- cbind(pop_df,e_vectors[,1:2]) %>%
    mutate(Ind = paste0('Ind',row_number()-1),
           hap = case_when(V1 >= 0 ~ 'Sex2',
                           V1 < 0 & V2 < 0.1 ~ 'Sex1a',
                           V1 < 0 & V2 >= 0.1 ~ 'Sex1b')) %>%
    arrange(hap)
...
# colnames(plotData) <- c("hap", "Ind", "PC1")
# head(plotData)
# 
# head(plotData)
...
plot.table.meta <- left_join(plot.table, plotData, by = "Ind") %>%
  mutate(hap = factor(hap, levels = c('Sex1a', 'Sex1b', 'Sex2')))

GENOTITLE <- "Sex Haplotype"
...
plot.table.meta$Ind <- factor(plot.table.meta$Ind,
                             levels=unique((plot.table.meta$Ind)[order(plot.table.meta$hap)]))

# #order based on PC1
# plot.table.meta$Ind <- factor(plot.table.meta$Ind, 
#                               levels=unique((plot.table.meta$Ind)[order(plot.table.meta$hap, -plot.table.meta$PC1)]))
...
pop_heatmap <- ggplot(plot.table.meta, aes(Locality_text,Ind, fill=hap)) + 
  geom_tile() + 
  theme_minimal() + 
...
#geno_plot
jpeg(paste0("./figures/genotype/boreogadus_",name,"_GL.jpeg"), width = 20, height = 5, res = 150, units = "in")
print(geno_plot)
dev.off()
