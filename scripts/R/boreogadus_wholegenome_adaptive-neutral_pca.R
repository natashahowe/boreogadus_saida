...
for(i in 1:length(markertypes)){
  ...
  for(j in 1:length(alphas)){
    ...
    # read in the covariance matrix
     cov <- as.matrix(read.table(paste0("./results/pca/boreogadus_wholegenome_",DATE,"_",markertype,"_alpha",alpha,".cov")))
    ...
    pc12 <- ggplot(data = pca.vectors, aes(x=V1, y=V2, fill = Pop), color = 'gray20') + 
      ...
      ggtitle(paste0("Pond Inlet ",markertype," Markers (alpha=",alpha,")")) +
      labs(x = paste0("PC1 (",round(varPC1, digits = 2),"%)"), 
           y= paste0("PC2 (",round(varPC2, digits = 2),"%)")) 
    pc12
    ggsave(filename = paste0("./figures/pca/",DATE,"/boreogadus_wholegenome_",markertype,"_alpha",alpha,"_pc1v2.jpeg"), 
    ...
    pc13
    ggsave(filename = paste0("./figures/pca/",DATE,"/boreogadus_wholegenome_",markertype,"_alpha",alpha,"_pc1v3.jpeg"), 
